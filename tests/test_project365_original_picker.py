...
class Project365OriginalPickerTests(unittest.TestCase):
    def test_crop_month_window_returns_anchor_and_next_populated_month(self) -> None:
        entries = [
            {"entry_id": "project365:2016-10-31", "entry_date": "2016-10-31"},
            {"entry_id": "project365:2016-11-01", "entry_date": "2016-11-01"},
            {"entry_id": "project365:2016-11-30", "entry_date": "2016-11-30"},
            {"entry_id": "project365:2017-01-02", "entry_date": "2017-01-02"},
            {"entry_id": "project365:2017-02-01", "entry_date": "2017-02-01"},
        ]

        window = picker._crop_month_window(entries, anchor_month="2016-11", month_count=2)

        self.assertEqual(window["months"], ["2016-11", "2017-01"])
        self.assertEqual(
            [entry["entry_id"] for entry in window["entries"]],
            ["project365:2016-11-01", "project365:2016-11-30", "project365:2017-01-02"],
        )
        self.assertEqual(window["previous_month"], "2016-10")
        self.assertEqual(window["next_month"], "2017-02")

    def test_crop_month_window_uses_nearest_available_month_at_scope_edges(self) -> None:
        entries = [
            {"entry_id": "project365:2016-11-01", "entry_date": "2016-11-01"},
            {"entry_id": "project365:2017-01-02", "entry_date": "2017-01-02"},
        ]

        before = picker._crop_month_window(entries, anchor_month="2016-01", month_count=2)
        after = picker._crop_month_window(entries, anchor_month="2018-01", month_count=2)

        self.assertEqual(before["months"], ["2016-11", "2017-01"])
        self.assertEqual(after["months"], ["2017-01"])
        self.assertEqual(after["previous_month"], "2016-11")
        self.assertEqual(after["next_month"], "")
    ...
    def test_picker_summary_counts_pending_accepts_and_rejects(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(
                ...
                {"accepted": 1, "rejected": 1, "associated": 0, "fallback": 0},
            )
    ...
    def test_picker_links_multiple_associated_photos_without_replacing_selected_original(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(
                state.summary()["pending_decisions"],
                {"accepted": 1, "rejected": 0, "associated": 2, "fallback": 0},
            )
    ...
    def test_picker_unlinks_associated_photo_without_clearing_selected_original(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(
                state.summary()["pending_decisions"],
                {"accepted": 1, "rejected": 0, "associated": 0, "fallback": 0},
            )
    ...
    def test_crop_entry_detail_returns_only_selected_original(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(crop_entry["candidates"][0]["dimensions"], "80 x 60")
    ...
    def test_picker_lists_fallback_entries_and_can_replace_with_index_candidate(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            base = Path(temp_dir)
            canonical_root = _import_sample(base, {"1998-04-12.png": _tiny_png()})
            ...
            library_root = _project_originals_root(base)
            candidate_path = library_root / "1998-04-12 better original.jpg"
            ...
            summary = state.summary()
            fallback_page = state.entry_page(status="fallback", limit=None)
            fallback_detail = state.entry_detail("project365:1998-04-12")
            expanded = state.expand_default_date_range("project365:1998-04-12")
            selected = state.save_decision(
                "project365:1998-04-12",
                expanded["entry"]["candidates"][0]["path"],
                "use_external_original",
                "found better original",
            )
            apply_summary = state.apply_decisions()
            with sqlite3.connect(canonical_root / "canonical.db") as connection:
                ...
                original_count = connection.execute(
                    "SELECT COUNT(*) FROM media_assets WHERE role = 'external_original_reference'"
                ).fetchone()[0]

        self.assertEqual(summary["status_counts"]["fallback"], 1)
        self.assertEqual(fallback_page["returned_count"], 1)
        self.assertEqual(fallback_page["entries"][0]["status"], "fallback")
        self.assertEqual(fallback_detail["status"], "fallback")
        self.assertEqual(fallback_detail["candidate_count"], 0)
        self.assertEqual(expanded["added_count"], 1)
        self.assertEqual(expanded["entry"]["candidate_count"], 1)
        self.assertEqual(selected["status"], "selected")
    ...
    def test_picker_fallback_page_limit_stops_database_summary_scan(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            queue_path = canonical_root / "exports" / "verification_reports" / "original_photo_external_search_queue.csv"
            _write_queue(queue_path)
            for date_text in ("1998-04-12", "1998-04-13", "1998-04-14"):
                _insert_picker_fallback(canonical_root, f"project365:{date_text}")
            state = picker.PickerState(
                picker.PickerConfig(canonical_root=canonical_root, queue_path=queue_path)
            )
            original_summary = state._database_entry_summary
            summarized_entry_ids = []

            def counted_summary(entry_id: str, fallback_only: bool = False) -> dict | None:
                summarized_entry_ids.append(entry_id)
                return original_summary(entry_id, fallback_only=fallback_only)

            state._database_entry_summary = counted_summary  # type: ignore[method-assign]

            first_page = state.entry_page(status="fallback", limit=2, offset=0)

        self.assertEqual([entry["entry_date"] for entry in first_page["entries"]], ["1998-04-12", "1998-04-13"])
        self.assertTrue(first_page["has_more"])
        self.assertEqual(summarized_entry_ids, ["project365:1998-04-12", "project365:1998-04-13"])

    def test_picker_recovers_orphaned_project_addition_for_fallback_entry(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            base = Path(temp_dir)
            canonical_root = _import_sample(base, {"1998-04-12.png": _tiny_png()})
            queue_path = canonical_root / "exports" / "verification_reports" / "original_photo_external_search_queue.csv"
            ...
            addition_dir.mkdir(parents=True)
            addition_path = addition_dir / "1998-04-12 000001 (Project365 project file addition).jpg"
            addition_path.write_bytes(_jpeg_with_dimensions(30, 20))
            state = picker.PickerState(
                picker.PickerConfig(canonical_root=canonical_root, queue_path=queue_path)
            )
        ...
        self.assertEqual(fallback_page["returned_count"], 0)
        self.assertEqual(accepted_page["returned_count"], 1)
        self.assertEqual(accepted_page["entries"][0]["status"], "selected")
        self.assertEqual(detail["status"], "selected")
        self.assertEqual(detail["candidate_count"], 1)
        self.assertEqual(detail["candidates"][0]["path"], str(addition_path.resolve()))
    ...
    def test_apply_skips_unknown_test_targets_without_blocking_valid_decisions(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(result["removed_unknown_media_entries"], 1)
            self.assertEqual(state.summary()["pending_decisions"]["accepted"], 0)
            self.assertIsNone(state.entry_detail("project365:2026-09-14"))
            with sqlite3.connect(canonical_root / "canonical.db") as connection:
                confirmed_entries = [
                    row[0]
                    for row in connection.execute(
    ...
    def test_crop_entries_include_project365_fallback_targets_without_external_original(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            base = Path(temp_dir)
            canonical_root = _import_sample(base, {"1998-04-12.png": _tiny_png()})
            ...
            self.assertEqual([entry["entry_id"] for entry in entries], ["project365:1998-04-12"])
            self.assertEqual(entries[0]["status"], "fallback")
            candidate_path = entries[0]["candidate_path"]
            self.assertTrue(candidate_path.endswith("1998-04-12.png"))
            ...
            self.assertTrue(cropped["crop_has_crop"])
            self.assertEqual(cropped["status"], "fallback")
            self.assertEqual(state.pending_crop_commits()["pending_count"], 1)
            ...
            self.assertEqual(json.loads(transformation_text)["review_crop"]["source"], "manual")
            self.assertEqual(state.crop_entries(crop_filter="missing"), [])
    ...
    def test_crop_estimate_batch_applies_project365_fallback_target_estimates(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            base = Path(temp_dir)
            canonical_root = _import_sample(base, {"1998-04-12.png": _tiny_png()})
            queue_path = base / "missing_review_queue.csv"
            ...
            candidate_path = state.crop_entries(crop_filter="missing")[0]["candidate_path"]
            suggestion = crop_align.CropSuggestion(
                x=0,
                y=0,
                width=1,
            ...
            self.assertTrue(estimated_entries[0]["estimate_batch_complete"])
            self.assertEqual(estimated_entries[0]["estimate_batch_number"], 1)
            reopened = picker.PickerState(picker.PickerConfig(
                canonical_root=canonical_root, queue_path=queue_path
            ))
            self.assertTrue(reopened.crop_entries(crop_filter="estimated")[0]["estimate_batch_complete"])
            self.assertEqual(state.pending_crop_commits()["pending_count"], 0)
    ...
    def test_linked_crop_estimates_are_visible_and_batches_finish_incrementally(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            base = Path(temp_dir)
            canonical_root = _import_sample(base, {"1998-04-12.png": _tiny_png()})
            _write_queue(base / "queue.csv")
            state = picker.PickerState(picker.PickerConfig(
                canonical_root=canonical_root, queue_path=base / "queue.csv"
            ))
            targets = [
                {"entry_id": f"project365:2000-01-0{index}", "candidate_path": str(base / f"{index}.jpg"),
                 "candidate_role": "external_original_associated_photo"}
                for index in range(1, 4)
            ]
            self.assertTrue(picker._crop_filter_matches(
                {"candidate_role": "external_original_associated_photo", "crop_has_crop": True,
                 "crop_source": "estimated", "crop_commit_pending": False}, "estimated"
            ))
            with (
                mock.patch.object(state, "crop_entries", return_value=targets),
                mock.patch.object(picker, "loaded_model", return_value="qwen/qwen3-vl-8b@4bit"),
                mock.patch.object(picker, "VISUAL_CROP_BATCH_SIZE", 2),
                mock.patch.object(state, "_crop_candidate_has_review_crop", return_value=False),
                mock.patch.object(state, "_estimated_crop_for_candidate", return_value={"x": 0, "y": 0, "size": 1}),
                mock.patch.object(state, "_stage_missing_crop_estimate", return_value=True) as stage,
                mock.patch.object(state, "_mark_estimate_batch_complete") as complete,
            ):
                job = state.start_crop_estimate_batch(apply_estimates=True)
                finished = _wait_for_crop_estimate_job(state, job["id"])
            self.assertEqual(finished["status"], "pass")
            self.assertEqual(finished["target_count"], 3)
            self.assertEqual(finished["estimated_count"], 3)
            self.assertEqual(finished["completed_batch_count"], 2)
            self.assertEqual(stage.call_count, 3)
            self.assertEqual(complete.call_count, 2)

    def test_linked_crop_batch_stops_when_local_model_closes(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            base = Path(temp_dir)
            canonical_root = _import_sample(base, {"1998-04-12.png": _tiny_png()})
            state = picker.PickerState(picker.PickerConfig(
            ...
            with (
                mock.patch.object(state, "crop_entries", return_value=targets),
                mock.patch.object(picker, "loaded_model", side_effect=[
                    "loaded", ValueError("Local model closed")
                ]),
                mock.patch.object(state, "_crop_candidate_has_review_crop", return_value=False),
                mock.patch.object(state, "_estimated_crop_for_candidate", side_effect=[
                    {"x": 0, "y": 0, "size": 1}, ConnectionError("model disconnected")
                ]) as estimate,
                mock.patch.object(state, "_stage_missing_crop_estimate", return_value=True),
                mock.patch.object(state, "_mark_estimate_batch_complete") as complete,
            ):
                job = state.start_crop_estimate_batch(apply_estimates=True)
                finished = _wait_for_crop_estimate_job(state, job["id"])
            ...
            self.assertEqual(finished["estimated_count"], 1)
            self.assertEqual(finished["completed_batch_count"], 1)
            self.assertEqual(finished["message"], "Local model closed")
            self.assertEqual(estimate.call_count, 2)
            complete.assert_called_once()

    def test_batch_estimate_does_not_replace_manual_save_during_analysis(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            base = Path(temp_dir)
            canonical_root = _import_sample(base, {"1998-04-12.png": _tiny_png()})
            _write_queue(base / "queue.csv")
            ...
            entry = state.crop_entries(crop_filter="missing")[0]
            crop = {"x": 0, "y": 0, "size": 1, "candidate_width": 1, "candidate_height": 1}

            def save_manual_during_analysis(entry_id, candidate_path):
                state.save_crop(entry_id, candidate_path, crop, source="manual")
                return dict(crop)

            with mock.patch.object(state, "_estimated_crop_for_candidate", side_effect=save_manual_during_analysis):
                job = state.start_crop_estimate_batch(apply_estimates=True)
                finished = _wait_for_crop_estimate_job(state, job["id"])
            self.assertEqual(finished["skipped_count"], 1)
            self.assertEqual(finished["estimated_count"], 0)
            detail = state.crop_entry_detail(entry["entry_id"], candidate_path=entry["candidate_path"])
            self.assertEqual(detail["candidates"][0]["review_crop_source"], "manual")
            self.assertEqual(state.pending_crop_commits()["pending_count"], 1)
    ...
    def test_crop_estimate_batch_stages_saved_estimate_database_crops_when_requested(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(state.pending_crop_commits()["pending_count"], 0)
            self.assertEqual(
                [entry["entry_id"] for entry in state.crop_entries(crop_filter="estimated")],
                ["project365:1998-04-12"],
            )
            self.assertEqual(state.crop_entries(crop_filter="missing"), [])
    ...
    def test_crop_estimate_batch_refreshes_unopened_saved_estimates_only(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            def estimate_after_marking_pending(
                entry_id: str,
                candidate_path_text: str,
                crop: dict[str, object] | None = None,
            ) -> dict[str, object]:
                estimated = original_estimate(entry_id, candidate_path_text, crop)
                state._mark_staged_crop_commit_pending(entry_id, candidate_path_text)
                return estimated

            with (
                mock.patch("project365_original_picker.suggest_crop", return_value=later_suggestion),
                mock.patch.object(state, "_estimated_crop_for_candidate", side_effect=estimate_after_marking_pending),
            ):
                job = state.start_crop_estimate_batch(
                    apply_estimates=True,
                    refresh_saved_estimates=True,
    ...
    def test_commit_staged_crops_can_be_limited_to_date_scope(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            state._mark_staged_crop_commit_pending(
                "project365:1998-04-12",
                str(source_root / "1998-04-12 original.jpg"),
            )
            self.assertEqual(state.pending_crop_commits()["pending_count"], 1)
    ...
    def test_crop_estimate_batch_reports_partial_failures(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            state._mark_staged_crop_commit_pending("project365:1998-04-12", str(source_root / "1998-04-12 original.jpg"))
            self.assertEqual(state.pending_crop_commits()["pending_count"], 1)
            self.assertFalse(state.crop_entry_detail("project365:1998-04-13")["crop_has_crop"])

    def test_single_crop_estimate_selects_best_quarter_turn_rotation(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            base = Path(temp_dir)
            canonical_root = _import_sample(base, {"1998-04-12.png": _tiny_png()})
            source_root = base / "external"
            ...
            reference_path = base / "portrait-target.jpg"
            reference_path.write_bytes(_jpeg_with_dimensions(60, 80))
            state._source_media["project365:1998-04-12"] = reference_path
            detail = state.entry_detail("project365:1998-04-12")
            candidate_path = detail["candidates"][0]["path"]
            ...
            with mock.patch("project365_original_picker.suggest_crop_for_rotations", return_value=(90.0, suggestion)) as suggest:
                result = state.suggest_crop_for_candidate("project365:1998-04-12", candidate_path)
    ...
    def test_crop_estimate_rotation_choices_use_aspect_when_not_square(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            base = Path(temp_dir)
            portrait = base / "portrait.jpg"
            landscape = base / "landscape.jpg"
            square = base / "square.jpg"
            portrait.write_bytes(_jpeg_with_dimensions(60, 80))
            landscape.write_bytes(_jpeg_with_dimensions(80, 60))
            square.write_bytes(_jpeg_with_dimensions(80, 80))

            self.assertEqual(
                picker._estimate_rotation_choices(portrait, landscape, 0),
                [90.0, -90.0],
            )
            self.assertEqual(
                picker._estimate_rotation_choices(square, landscape, 0),
                [0.0],
            )
            self.assertEqual(
                picker._estimate_rotation_choices(landscape, landscape, 0),
                [0.0],
            )
            self.assertEqual(
                picker._estimate_rotation_choices(portrait, landscape, 90),
                [90.0],
            )
    ...
    def test_single_crop_estimate_preserves_current_rotation(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            crop_entry = state.crop_entry_detail("project365:1998-04-12")
            candidate = crop_entry["candidates"][0]
            self.assertFalse(candidate.get("review_crop_source"))
            self.assertFalse(candidate.get("review_crop_rotation_degrees"))
            self.assertFalse(candidate.get("review_crop_fill_color"))
            self.assertEqual(state.pending_crop_commits()["pending_count"], 0)
    ...
    def test_pending_crop_rejection_is_original_review_target_before_commit(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            base = Path(temp_dir)
            canonical_root = _import_sample(base, {"1998-04-12.png": _tiny_png()})
            source_root = base / "external"
            source_root.mkdir()
            original = source_root / "1998-04-12 original.png"
            replacement = source_root / "1998-04-12 replacement.jpg"
            original.write_bytes(_tiny_png())
            summary = pipeline.build_external_original_search_queue(
                canonical_root=canonical_root,
                search_roots=[source_root],
            ...
            detail = state.entry_detail("project365:1998-04-12")
            original_path = detail["candidates"][0]["path"]
            state.save_decision(
                entry_id="project365:1998-04-12",
            ...
            state.reject_crop_original("project365:1998-04-12", original_path, "wrong original")
            replacement.write_bytes(_jpeg_with_dimensions(12, 9))
            updated_summary = pipeline.build_external_original_search_queue(
                canonical_root=canonical_root,
                search_roots=[source_root],
                review_queue_path=base / "missing_review_queue.csv",
            ...
            pending_state = picker.PickerState(
                picker.PickerConfig(canonical_root=canonical_root, queue_path=Path(updated_summary.search_queue_path))
            )

            entries = pending_state.entries(status="needs_action")
            pending_detail = pending_state.entry_detail("project365:1998-04-12")

            self.assertEqual([entry["entry_id"] for entry in entries], ["project365:1998-04-12"])
            self.assertEqual(entries[0]["status"], "needs_review")
            self.assertEqual([candidate["path"] for candidate in pending_detail["candidates"]], [str(replacement)])
            self.assertEqual(pending_state.pending_crop_commits()["pending_count"], 1)
            with sqlite3.connect(canonical_root / "canonical.db") as connection:
                reference_count = connection.execute(
                    """
                    SELECT COUNT(*)
    ...
    def test_picker_missing_queue_lists_database_entries_without_links(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            base = Path(temp_dir)
            canonical_root = _import_sample(
                base,
            ...
            queue_path = canonical_root / "exports" / "verification_reports" / "missing-queue.csv"
            with sqlite3.connect(canonical_root / "canonical.db") as connection:
                ...
                connection.execute(
                    ...
                    VALUES (?, ?, ?, ?, 'uncorrected', 'canonical_only', ?, ?)
                    """,
                    ("social:x_archive:synthetic-1", "1998-04-12", "x_archive", "synthetic social event", now, now),
                )
            state = picker.PickerState(
                picker.PickerConfig(canonical_root=canonical_root, queue_path=queue_path)
            )
            ...
            self.assertFalse(summary["queue_exists"])
            self.assertEqual(summary["status_counts"]["search_needed"], 2)
            self.assertEqual([entry["entry_date"] for entry in entries], ["1998-04-12", "1998-04-13"])
            self.assertNotIn("social:x_archive:synthetic-1", {entry["entry_id"] for entry in entries})
            self.assertTrue(all(entry["status"] == "search_needed" for entry in entries))
            self.assertIsNotNone(detail)
            self.assertEqual(detail["status"], "search_needed")
            self.assertEqual(detail["candidates"], [])
    ...
    def test_picker_can_mark_entry_as_project365_export_fallback(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(state.summary()["pending_decisions"]["fallback"], 1)
            self.assertEqual(state.summary()["pending_entry_counts"]["fallback"], 1)
            self.assertEqual(state.entries(status="needs_action"), [])
            self.assertEqual(
    ...
    def test_entry_scoped_fallback_survives_candidate_row_replacement_and_applies(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            base = Path(temp_dir)
            canonical_root = _import_sample(base, {"1998-04-12.png": _tiny_png()})
            source_root = base / "external"
            source_root.mkdir()
            source_root.joinpath("1998-04-12 candidate.png").write_bytes(_tiny_png())
            search_summary = pipeline.build_external_original_search_queue(
                canonical_root=canonical_root,
                search_roots=[source_root],
                review_queue_path=base / "missing_review_queue.csv",
            ...
            entry_id = "project365:1998-04-12"
            state._decision_overrides[entry_id] = {
                f"entry:{entry_id}:project365_export_png": {
                    "review_decision": "keep_project365_export",
                    "review_notes": "entry-scoped fallback",
                }
            }
            state._persist_decision_overrides()

            restarted = picker.PickerState(
                picker.PickerConfig(canonical_root=canonical_root, queue_path=queue_path)
            )

            self.assertEqual(restarted.entry_detail(entry_id)["status"], "fallback")
            self.assertEqual(restarted.summary()["pending_entry_counts"]["fallback"], 1)
            self.assertEqual(restarted.apply_decisions()["fallback_count"], 1)
    ...
    def test_picker_html_exposes_group_selection_controls_and_thumbnails(self) -> None:
        ...
        self.assertIn('class="entry-thumb-button" type="button" aria-label="Open ${escapeHtml(entry.entry_date)} target photo"', picker.PICKER_HTML)
        self.assertIn('const thumbButton = item.querySelector(".entry-thumb-button");', picker.PICKER_HTML)
        self.assertIn("thumbButton.onclick = () => loadEntry(entry.entry_id);", picker.PICKER_HTML)
        ...
        self.assertIn('id="sourceFrame" class="source-frame" draggable="false"', picker.PICKER_HTML)
        self.assertIn('id="sourceImage" class="source-image" alt="" draggable="false"', picker.PICKER_HTML)
        self.assertIn(".source-frame.is-draggable", picker.PICKER_HTML)
        self.assertIn("-webkit-user-select: none;", picker.PICKER_HTML)
        self.assertIn("-webkit-touch-callout: none;", picker.PICKER_HTML)
        self.assertIn("frame.draggable = false;", picker.PICKER_HTML)
        self.assertIn("image.draggable = Boolean(entry?.source_token);", picker.PICKER_HTML)
        self.assertNotIn("-webkit-user-drag: none;", picker.PICKER_HTML)
        self.assertNotIn("function handleSourceDragStart(event)", picker.PICKER_HTML)
        self.assertNotIn("function prepareSourceDragFile(entry, url)", picker.PICKER_HTML)
        self.assertNotIn("transfer.items.add(state.sourceDrag.file)", picker.PICKER_HTML)
        self.assertNotIn('addEventListener("dragstart", handleSourceDragStart)', picker.PICKER_HTML)
        self.assertIn('target?.closest?.("#sourceFrame")', picker.PICKER_HTML)
        self.assertLess(
        ...
        self.assertIn('state.entryLoadAll = params.get("load_all") === "1";', picker.PICKER_HTML)
        ...
        self.assertIn('Shortcut: K. Use the photo already stored in the Project365 entry instead of an external original', picker.PICKER_HTML)
        self.assertIn('id="privateCheckbox"', picker.PICKER_HTML)
        self.assertIn('id="privateCheckbox"', picker.CROP_HTML)
        self.assertIn('>Use Project365 photo (K)</button>', picker.PICKER_HTML)
        ...
        self.assertIn('return "Already main";', picker.PICKER_HTML)
        self.assertIn('Already selected as the main image for this diary entry', picker.PICKER_HTML)
        ...
        self.assertIn('fallback: "needs better original"', picker.PICKER_HTML)
        ...
        self.assertIn('id="fallbackDecisionCount"', picker.PICKER_HTML)
        ...
        self.assertIn("grid-template-columns: minmax(132px, 1fr) minmax(132px, 1fr) max-content auto", picker.PICKER_HTML)
        self.assertIn(".manual-date-search input", picker.PICKER_HTML)
        ...
        self.assertIn('setCrawlStatus(`Searching ${scopeLabel} from ${scope.startDate} to ${scope.endDate}.`, true);', picker.PICKER_HTML)
        ...
        self.assertIn('window.addEventListener("focus"', picker.PICKER_HTML)
        ...
        self.assertIn("function droppedFileIsVideo(file)", picker.PICKER_HTML)
        self.assertIn("function droppedFileIsImage(file)", picker.PICKER_HTML)
        self.assertIn("function droppedLivePhotoStillFile(files)", picker.PICKER_HTML)
        self.assertIn("const livePhotoStill = droppedLivePhotoStillFile(files);", picker.PICKER_HTML)
        self.assertIn("await importDroppedPhotoFile(livePhotoStill, entry);", picker.PICKER_HTML)
        self.assertIn('["video/quicktime", "video/mp4", "video/x-m4v"].includes(type)', picker.PICKER_HTML)
        self.assertIn("Safari did not expose one usable photo or movie file for this drop.", picker.PICKER_HTML)
        ...
        self.assertIn('if (candidate.selected) return "Already main";', picker.PICKER_HTML)
        ...
        self.assertIn('saveButton.textContent = "Saving...";', picker.PICKER_HTML)
        self.assertIn('document.getElementById("videoFrameHelp").textContent = "Saving selected movie screenshots.";', picker.PICKER_HTML)
        self.assertIn("void discardVideoFrameSession(review);", picker.PICKER_HTML)
        self.assertIn("closeVideoFrameChooser({discard: false});", picker.PICKER_HTML)
        ...
        self.assertIn("Drop one or more image files, or a local file path.", picker.PICKER_HTML)
        ...
        self.assertIn('params.delete("load_all");', picker.PICKER_HTML)
    ...
    def test_link_drop_popup_uses_associated_photo_import(self) -> None:
        ...
        self.assertIn("function droppedLivePhotoStillFile(files)", html)
        self.assertIn("function linkedPhotoDropFiles(files)", html)
        self.assertIn("const livePhotoStill = droppedLivePhotoStillFile(files);", html)
        self.assertIn("const filesToImport = linkedPhotoDropFiles(files);", html)
        self.assertIn("for (const file of filesToImport) detail = await importLinkedFile(file);", html)
        self.assertIn("event.stopPropagation();", html)
        self.assertIn('localStorage.setItem("project365-linked-photo-refresh"', html)
        self.assertIn('window.addEventListener("drop", handleDrop, true);', html)
        self.assertIn('window.addEventListener("dragover", activateDrop, true);', html)
        self.assertIn("Linked ${count} associated attachments", html)
    ...
    def test_picker_links_absolute_drop_path_for_database_only_entry(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(detail["accepted_count"], 1)
            self.assertEqual(detail["selected_count"], 1)
            self.assertEqual(detail["pending_commit_count"], 0)
            self.assertEqual(detail["associated_count"], 1)
            selected = [candidate for candidate in detail["candidates"] if candidate["selected"]]
            associated = [
                candidate
                for candidate in detail["candidates"]
                if candidate["review_decision"] == "external_original_associated_photo"
            ]
            self.assertEqual(len(selected), 1)
            self.assertEqual(selected[0]["database_confirmed_role"], "external_original_reference")
    ...
    def test_picker_copies_dropped_image_into_managed_source_folder(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            def fake_metadata(paths: list[Path]) -> dict[str, picker.ExiftoolPhotoMetadata]:
                return {
                    str(paths[0].resolve()): picker.ExiftoolPhotoMetadata(
                        capture_timestamp="1998-04-12T08:09:10",
                        capture_timestamp_source="exif_datetime_original",
                        gps_latitude=25.0962305555556,
                        gps_longitude=121.512,
                        gps_source="composite_gps",
                    )
                }
            ...
            with (
                ...
                mock.patch.object(picker, "_exiftool_photo_metadata", side_effect=fake_metadata),
            ):
                detail = state.add_copied_candidate(
                    "project365:1998-04-12",
                    "known-original.png",
            ...
            self.assertEqual(detail["candidates"][0]["media_creation_dates"], "1998-04-12")
            self.assertEqual(detail["candidates"][0]["capture_timestamp"], "1998-04-12T08:09:10")
            self.assertEqual(detail["candidates"][0]["capture_timestamp_source"], "exif_datetime_original")
            self.assertEqual(detail["candidates"][0]["gps_latitude"], "25.096230555556")
            self.assertEqual(detail["candidates"][0]["gps_longitude"], "121.512")
            self.assertEqual(detail["candidates"][0]["gps_source"], "composite_gps")
            self.assertEqual(detail["candidates"][0]["has_gps"], "1")
            self.assertTrue(detail["candidates"][0]["has_embedded_geolocation"])
    ...
    def test_picker_prepares_and_imports_dropped_video_frames(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            _wait_for_path_absent(frame_dir)

    def test_picker_video_drop_route_uses_content_type_when_name_has_no_suffix(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            base = Path(temp_dir)
            canonical_root = _import_sample(base, {"1998-04-12.png": _tiny_png()})
            queue_path = canonical_root / "exports" / "verification_reports" / "queue.csv"
            ...
            try:
                ...
                with mock.patch.object(
                    picker.PickerState,
                    "prepare_video_frame_choices",
                    return_value={"kind": "video_frame_choices", "session_id": "session"},
                ) as prepare_mock:
                    detail = _post_bytes(
                        f"{base_url}/api/import-dropped-candidate",
                        b"movie",
                        {
                            "content-type": "video/quicktime",
                            "x-entry-id": "project365%3A1998-04-12",
                            "x-file-name": "Photos export",
                        },
                    )
                self.assertEqual(detail["kind"], "video_frame_choices")
                prepare_mock.assert_called_once()
                _, kwargs = prepare_mock.call_args
                self.assertEqual(kwargs["content_type"], "video/quicktime")
                self.assertEqual(kwargs["filename"], "Photos export")
                self.assertEqual(Path(kwargs["upload_path"]).suffix, ".mov")
            finally:
                server.shutdown()
                server.server_close()
                thread.join(timeout=5)
    ...
    def test_picker_copies_link_drop_for_database_only_entry(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(len(crop_entries), 2)
            self.assertEqual(
                {entry["candidate_path"] for entry in crop_entries},
                {persisted_selected[0]["path"], persisted_associated[0]["path"]},
            )
            self.assertEqual(len(state.crop_entries(crop_filter="missing")), 2)
            associated_path = persisted_associated[0]["path"]
            associated_crop = state.crop_entry_detail("project365:1998-04-12", candidate_path=associated_path)
            self.assertEqual(associated_crop["candidates"][0]["path"], associated_path)
            self.assertFalse(associated_crop["source_exists"])
            visual_crop = {
                "x": 0, "y": 0, "size": 15,
                "candidate_width": 20, "candidate_height": 15,
                "rotation_degrees": 1.5,
            }
            with mock.patch.object(picker, "estimate_visual_crop", return_value=visual_crop) as visual:
                suggested = state.suggest_crop_for_candidate("project365:1998-04-12", associated_path)
            self.assertEqual(suggested["crop"], visual_crop)
            visual.assert_called_once_with(Path(associated_path))
            staged_linked = state.save_crop(
                "project365:1998-04-12", associated_path,
                {"x": 0, "y": 0, "size": 15, "candidate_width": 20, "candidate_height": 15},
            )
            self.assertEqual(staged_linked["candidates"][0]["path"], associated_path)
            self.assertTrue(staged_linked["crop_has_crop"])
            self.assertEqual(len(state.crop_entries(crop_filter="missing")), 1)
            self.assertEqual(state.commit_staged_crops()["saved_count"], 1)
            self.assertEqual(
                [entry["candidate_path"] for entry in state.crop_entries(crop_filter="confirmed")],
                [associated_path],
            )
            self.assertEqual(state.crop_entries(crop_filter="estimated"), [])
            estimate_job = state.start_crop_estimate_batch()
            self.assertEqual(estimate_job["target_count"], 1)
            self.assertEqual(_wait_for_crop_estimate_job(state, estimate_job["id"])["processed_count"], 1)
    ...
    def test_crop_unlink_only_selected_linked_photo_and_its_derivative(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            base = Path(temp_dir)
            canonical_root = _import_sample(base, {"1998-04-12.png": _tiny_png()})
            queue_path = canonical_root / "exports" / "verification_reports" / "queue.csv"
            _write_queue(queue_path)
            _append_search_entry(queue_path)
            config = picker.PickerConfig(canonical_root=canonical_root, queue_path=queue_path)
            state = picker.PickerState(config)
            entry_id = "project365:1998-04-12"
            with mock.patch.object(picker, "_set_project_addition_timestamps"):
                state.add_copied_candidate(entry_id, "main.jpg", "image/jpeg", _jpeg_with_dimensions(30, 20))
            state.apply_decisions()
            state = picker.PickerState(config)
            with mock.patch.object(picker, "_set_project_addition_timestamps"):
                state.add_copied_candidate(entry_id, "linked-one.jpg", "image/jpeg", _jpeg_with_dimensions(20, 15), associate=True)
                state.add_copied_candidate(entry_id, "linked-two.jpg", "image/jpeg", _jpeg_with_dimensions(25, 15), associate=True)
            self.assertEqual(state.apply_decisions()["associated_count"], 2)
            with sqlite3.connect(canonical_root / "canonical.db") as connection:
                linked = connection.execute(
                    "SELECT id, storage_path FROM media_assets WHERE entry_id = ? AND role = 'external_original_associated_photo' ORDER BY storage_path",
                    (entry_id,),
                ).fetchall()
                source_id, source_path = linked[0]
                import_batch_id = connection.execute("SELECT import_batch_id FROM media_assets WHERE id = ?", (source_id,)).fetchone()[0]
                now = dt.datetime.now(dt.UTC).isoformat()
                connection.execute(
                    """INSERT INTO media_assets (id, entry_id, role, source_file_id, internal_filename,
                        storage_path, sha256, byte_size, mime_type, status, review_status,
                        selected_default, transformation_json, import_batch_id, created_at, updated_at)
                       VALUES (?, ?, 'diarium_associated_derivative', NULL, 'working-copy.jpg', ?, ?, 1,
                        'image/jpeg', 'available', 'confirmed', 0, ?, ?, ?, ?)""",
                    (source_id + ':diarium_associated_derivative:test', entry_id, str(base / 'working-copy.jpg'),
                     'a' * 64, json.dumps({'source_media_asset_id': source_id}), import_batch_id, now, now),
                )
            result = state.reject_crop_original(entry_id, source_path, "unlink from crop")
            self.assertTrue(result["staged"])
            self.assertEqual(result["unlinked_count"], 1)
            self.assertEqual(state.pending_crop_commits()["pending_count"], 1)
            state = picker.PickerState(config)
            self.assertEqual(state.pending_crop_commits()["pending_count"], 1)
            self.assertNotIn(source_path, {row["candidate_path"] for row in state.crop_entries(crop_filter="all")})
            with sqlite3.connect(canonical_root / "canonical.db") as connection:
                self.assertEqual(connection.execute("SELECT COUNT(*) FROM media_assets WHERE id = ?", (source_id,)).fetchone()[0], 1)
            committed = state.commit_staged_crops()
            self.assertEqual(committed["unlinked_count"], 1)
            self.assertEqual(committed["rejected_count"], 0)
            self.assertEqual(committed["remaining_count"], 0)
            self.assertTrue(Path(source_path).exists())
            with sqlite3.connect(canonical_root / "canonical.db") as connection:
                self.assertEqual(connection.execute("SELECT COUNT(*) FROM media_assets WHERE id = ?", (source_id,)).fetchone()[0], 0)
                self.assertEqual(dict(connection.execute(
                    "SELECT role, COUNT(*) FROM media_assets WHERE entry_id = ? AND role IN ('external_original_reference', 'external_original_associated_photo', 'external_original_rejected') GROUP BY role",
                    (entry_id,),
                )), {"external_original_reference": 1, "external_original_associated_photo": 1})
                self.assertEqual(connection.execute("SELECT status FROM media_assets WHERE id = ?", (source_id + ':diarium_associated_derivative:test',)).fetchone()[0], "not_ready")
            self.assertEqual(len(state.crop_entries(crop_filter="all")), 2)

    def test_picker_link_drop_preserves_pending_fallback_for_database_only_entry(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            base = Path(temp_dir)
            canonical_root = _import_sample(base, {"1998-04-12.png": _tiny_png()})
            queue_path = canonical_root / "exports" / "verification_reports" / "queue.csv"
            ...
            self.assertEqual(detail["fallback_count"], 1)
            self.assertEqual(detail["associated_count"], 1)
            self.assertEqual(
                state.summary()["pending_decisions"],
                {"accepted": 0, "rejected": 0, "associated": 1, "fallback": 1},
            )
    ...
    def test_picker_formats_candidate_folder_labels(self) -> None:
        ...
        self.assertIn("const loadAll = explicitScope || state.entryLoadAll;", picker.PICKER_HTML)
        self.assertIn("loadAll && payload.has_more", picker.PICKER_HTML)
        self.assertIn("if (loadAll) {", picker.PICKER_HTML)
        self.assertIn("state.entries = entries;", picker.PICKER_HTML)
        self.assertIn("state.entryHasMore = Boolean(payload.has_more);", picker.PICKER_HTML)
        self.assertIn('entries.sort((left, right) => String(left.entry_date || "").localeCompare(String(right.entry_date || ""))', picker.PICKER_HTML)
        self.assertIn("state.entryHasMore = loadAll ? false : Boolean(payload.has_more);", picker.PICKER_HTML)
        self.assertIn('id="loadMoreEntries"', picker.PICKER_HTML)
        self.assertIn('id="loadAllEntries"', picker.PICKER_HTML)
        self.assertIn("grid-template-columns: 7fr 3fr;", picker.PICKER_HTML)
        self.assertIn("function setEntryLoadAllUrl(enabled)", picker.PICKER_HTML)
        self.assertIn('params.set("load_all", "1");', picker.PICKER_HTML)
        self.assertIn('params.set("batch", batchId);', picker.PICKER_HTML)
        self.assertIn("function entryPageParams(offset)", picker.PICKER_HTML)
        self.assertIn("function appendEntryPage(payload)", picker.PICKER_HTML)
        self.assertIn("async function loadMoreEntries(loadAll = false)", picker.PICKER_HTML)
        self.assertIn("renderBatchSummary();", picker.PICKER_HTML)
        self.assertIn("while (loadAll && state.entryHasMore)", picker.PICKER_HTML)
        self.assertIn("setEntryLoadAllUrl(true);", picker.PICKER_HTML)
        self.assertIn('document.getElementById("loadAllEntries").onclick', picker.PICKER_HTML)
        ...
        self.assertIn('placeholder="yyyy-mm-dd or yyyy-mm"', picker.PICKER_HTML)
        self.assertIn('placeholder="optional end"', picker.PICKER_HTML)
        self.assertIn("grid-template-columns: minmax(0, 1fr) minmax(0, 1fr) auto;", picker.PICKER_HTML)
        self.assertIn("function entryDateScopeLabel(dateTexts)", picker.PICKER_HTML)
        self.assertIn("function dateScopeFromInputValues(startValue, endValue = \"\")", picker.PICKER_HTML)
        self.assertIn("function parseDateScopeValue(value)", picker.PICKER_HTML)
        ...
        self.assertIn('state.urlEntryDates = scope.dates;', picker.PICKER_HTML)
        self.assertIn('replaceEntryDateScopeUrl(scope.dates);', picker.PICKER_HTML)
        self.assertIn('await loadEntries("", false, scope.startDate);', picker.PICKER_HTML)
    ...
    def test_crop_html_exposes_separate_crop_confirmation_controls(self) -> None:
        ...
        self.assertIn('const estimatedBody = await fetchJson(cropEntriesUrl("estimated", anchorMonth));', picker.CROP_HTML)
        ...
        self.assertIn('const allBody = await fetchJson(cropEntriesUrl("all", anchorMonth));', picker.CROP_HTML)
        ...
        self.assertIn('function cropEntriesUrl(cropFilter, anchorMonth = "")', picker.CROP_HTML)
        ...
        self.assertIn('id="cropStartDate" type="text"', picker.CROP_HTML)
        self.assertIn('placeholder="yyyy-mm-dd or yyyy-mm"', picker.CROP_HTML)
        self.assertIn('placeholder="optional end"', picker.CROP_HTML)
        self.assertIn('id="applyCropDateScopeButton"', picker.CROP_HTML)
        self.assertIn("if (!applyCropDateScopeControls()) return;", picker.CROP_HTML)
        self.assertIn("function dateScopeFromInputValues(startValue, endValue = \"\")", picker.CROP_HTML)
        self.assertIn("function parseDateScopeValue(value)", picker.CROP_HTML)
        ...
        self.assertIn("function cropImageUrl(token, filename, path, maxSize = 0)", picker.CROP_HTML)
        self.assertIn('cropImageUrl(entry.candidate_token, entry.candidate_filename, entry.candidate_path, 96)', picker.CROP_HTML)
        self.assertIn('originalImage.src = cropImageUrl(candidate.token, candidate.filename, candidate.path);', picker.CROP_HTML)
        self.assertIn("function cropCandidateDimensions(candidate, image)", picker.CROP_HTML)
        self.assertIn("const intrinsic = cropCandidateDimensions(state.selectedCandidate, image);", picker.CROP_HTML)
        self.assertIn('class="crop-preview-actions"', picker.CROP_HTML)
        self.assertIn('id="privateStatus" class="crop-private-status"', picker.CROP_HTML)
        self.assertIn(".crop-private-status {", picker.CROP_HTML)
        self.assertIn("text-overflow: ellipsis;", picker.CROP_HTML)
        self.assertIn("function setCropPrivateStatus(message)", picker.CROP_HTML)
        self.assertIn("status.title = text;", picker.CROP_HTML)
        ...
        self.assertIn('} else if (key === "e") {', picker.CROP_HTML)
        self.assertIn('handled = runCropShortcutButton("suggestCropButton", estimateCropForCurrentCandidate);', picker.CROP_HTML)
        self.assertIn('id="previousCropEntryButton"', picker.CROP_HTML)
        self.assertIn('id="nextCropEntryButton"', picker.CROP_HTML)
        self.assertIn('id="cropStateFlag" class="crop-state-flag missing"', picker.CROP_HTML)
        self.assertIn("function cropStateFlagForEntry(entry, candidate)", picker.CROP_HTML)
        self.assertIn('if (source === "manual") return [pending ? "User saved crop pending" : "User saved crop", "manual"];', picker.CROP_HTML)
        self.assertIn("cropNavigationHistory: []", picker.CROP_HTML)
        self.assertIn("cropNavigationResume: null", picker.CROP_HTML)
        self.assertIn("function loadCropNavigationHistoryEntry(previous)", picker.CROP_HTML)
        self.assertIn('filterControl.value = "all";', picker.CROP_HTML)
        self.assertIn("rememberCropNavigationResume(previous.entry_id, resumeFilter, resumeEntryId, previous.candidate_path, resumeCandidatePath);", picker.CROP_HTML)
        self.assertIn("function consumeCropNavigationResume(entryId)", picker.CROP_HTML)
        self.assertIn('filterControl.value = resume.cropFilter;', picker.CROP_HTML)
        self.assertIn("await loadEntries(resume.resumeEntryId, {preferredCandidatePath: resume.resumeCandidatePath});", picker.CROP_HTML)
        self.assertIn("Crop list changed to All crop targets.", picker.CROP_HTML)
        self.assertIn('document.getElementById("summary").textContent = `Loading All crop targets. ${state.cropNavigationNotice}`;', picker.CROP_HTML)
        self.assertIn("anchorMonth: cropMonth(previous.entry_date)", picker.CROP_HTML)
        self.assertIn("preserveCurrentEntry: true", picker.CROP_HTML)
        self.assertIn("if (options.preserveCurrentEntry && state.currentEntry?.entry_id === state.selectedEntryId", picker.CROP_HTML)
        self.assertIn("cropWindowAnchorMonth: \"\"", picker.CROP_HTML)
        self.assertIn("cropWindowPreviousMonth: \"\"", picker.CROP_HTML)
        self.assertIn("cropWindowNextMonth: \"\"", picker.CROP_HTML)
        self.assertIn('params.set("anchor_month", anchorMonth);', picker.CROP_HTML)
        self.assertIn('params.set("month_count", "2");', picker.CROP_HTML)
        self.assertIn("function selectCropEntry(entry)", picker.CROP_HTML)
        self.assertIn("function cropMonth(value)", picker.CROP_HTML)
        self.assertIn("previousButton.disabled = index <= 0 && !state.cropNavigationHistory.length && !state.cropWindowPreviousMonth;", picker.CROP_HTML)
        self.assertIn("nextButton.disabled = index < 0 || (index >= state.entries.length - 1 && !state.cropWindowNextMonth);", picker.CROP_HTML)
        self.assertIn("const boundaryMonth = direction < 0 ? state.cropWindowPreviousMonth : state.cropWindowNextMonth;", picker.CROP_HTML)
        self.assertIn("if (direction < 0 && state.cropNavigationHistory.length)", picker.CROP_HTML)
        self.assertIn("rememberCropNavigationHistory(savedEntry);", picker.CROP_HTML)
        ...
        self.assertIn('title="Shortcut: e. Estimate crop and rotation">Estimate crop (e)</button>', picker.CROP_HTML)
        ...
        self.assertIn("function sourceForCropSave()", picker.CROP_HTML)
        self.assertIn('function sourceForCropSave() {\n  return "manual";\n}', picker.CROP_HTML)
        self.assertNotIn("function cropsMatchForSourcePreservation", picker.CROP_HTML)
        ...
        self.assertIn('document.getElementById("cropStatus").textContent = "Estimating crop and rotation."', picker.CROP_HTML)
        self.assertIn("crop: existingCrop", picker.CROP_HTML)
        self.assertIn("Estimated crop loaded as a draft. Press Save crop to stage it.", picker.CROP_HTML)
        ...
        self.assertIn("const selected = boundarySelection", picker.CROP_HTML)
        self.assertIn("|| cropEntryById(requested, options.preferredCandidatePath || state.selectedCandidatePath)", picker.CROP_HTML)
        ...
        self.assertIn("fetchJson(cropEntriesUrl(cropFilter, anchorMonth))", picker.CROP_HTML)
...
def _post_bytes(url: str, payload: bytes, headers: dict[str, str]) -> dict:
    request = urllib.request.Request(
        url,
        data=payload,
        headers=headers,
...
def _wait_for_path_absent(path: Path) -> None:
...
def _insert_picker_fallback(canonical_root: Path, entry_id: str) -> None:
    with sqlite3.connect(canonical_root / "canonical.db") as connection:
        import_batch_id = connection.execute(
            "SELECT import_batch_id FROM media_assets WHERE entry_id = ? AND role = 'project365_export_png'",
            (entry_id,),
        ).fetchone()[0]
        now = dt.datetime.now(dt.UTC).isoformat()
        ...
        connection.execute(
            ...
            (
                ...
                f"{entry_id}:external_original_fallback",
                sha256,
                json.dumps({"source": "external_original_fallback"}, sort_keys=True),
                import_batch_id,
                now,
                now,
            ),
